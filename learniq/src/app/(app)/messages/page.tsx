...
const ROLE_COLORS: Record<string, string> = {
  instructor: "bg-indigo-50 border-brand-200 dark:bg-indigo-900/20 dark:border-brand-800",
  ...
  return (
    <div className="h-full flex flex-col" dir={dir}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Message list */}
        <div className="hidden sm:flex sm:flex-col w-80 border-r border-slate-200 dark:border-slate-700 overflow-y-auto flex-shrink-0">
          {messages.length === 0 ? (
            ...
            messages.map(msg => (
              <button
                ...
                className={`w-full text-left p-4 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors ${selected?.id === msg.id ? "bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-l-brand-500" : ""}`}
              >
          ...
          {showCompose ? (
            ...
            <div className="p-6 max-w-3xl space-y-6">
              ...
              {(selected.ai_reply_suggestion_en || selected.ai_reply_suggestion_ar) && (
                <div className="rounded-xl border border-brand-200 bg-indigo-50 dark:bg-indigo-900/20 dark:border-brand-800 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="ml-auto flex gap-1">
                      <button
                        ...
                        className={`flex items-center gap-1 text-xs px-2 py-1 rounded-md transition-colors ${replyLang === "en" ? "bg-indigo-600 text-white" : "bg-white dark:bg-slate-800 text-brand-600 dark:text-brand-400 border border-brand-300 dark:border-brand-700"}`}
                      >
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 text-xs px-2 py-1 rounded-md transition-colors ${replyLang === "ar" ? "bg-indigo-600 text-white" : "bg-white dark:bg-slate-800 text-brand-600 dark:text-brand-400 border border-brand-300 dark:border-brand-700"}`}
                      >
